    ...
    <div className="navbar fixed top-0 z-[1000] px-4 md:px-12 backdrop-blur-md bg-black/20 font-['Orbitron']">
      ...
      <div className="navbar-start">
        <div className="dropdown">
          <div tabIndex={0} role="button" className="btn btn-ghost lg:hidden text-[#00ff88]">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="4 6h16M4 12h8m-8 6h16" />
            </svg>
          </div>
          <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-black/90 rounded-box w-52 text-white border border-white/10 uppercase">
            {navItems.map((item) => (
      ...
      <div className="navbar-center hidden lg:flex">
        <ul className="menu menu-horizontal px-1 text-white gap-4 text-sm tracking-widest uppercase">
          {navItems.map((item) => (
            <li key={item}>
              <a className="hover:text-[#00ff88] transition-all focus:text-[#00ff88]">{item}</a>
            </li>
          ))}
        </ul>
      </div>
